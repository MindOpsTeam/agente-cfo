...
if [[ -z "${NS_ACCESS_TOKEN:-}" ]]; then
    echo "❌ ${SKILL_NAME}: NS_ACCESS_TOKEN ausente"
    echo "   Execute: bash $(dirname "$0")/connect.sh"
    exit 1
fi
if [[ -z "${NS_STORE_ID:-}" ]]; then
    echo "❌ ${SKILL_NAME}: NS_STORE_ID ausente"
    echo "   Execute: bash $(dirname "$0")/connect.sh"
    exit 1
fi
echo "✅ Credenciais presentes (store_id: ${NS_STORE_ID})"
