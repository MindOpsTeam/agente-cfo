#!/usr/bin/env python3
"""OAuth token refresh para skill contaazul — Agente CFO. Sprint INT-2.

Uso: python3 oauth_refresh.py [--force]
Saída JSON stdout: {"ok": bool, "skill": "contaazul", "refreshed": bool, "expires_at": str|None}
...
TOKEN_URL = "https://api.contaazul.com/auth/token"
...
def _save_tokens(access_token: str, refresh_token: str, expiry: float) -> None:
    skip = {"CONTAAZUL_ACCESS_TOKEN", "CONTAAZUL_REFRESH_TOKEN", "CONTAAZUL_TOKEN_EXPIRY"}
    lines = []
...
def _push_to_panel(access_token: str, refresh_token: str, expires_in: int) -> None:
    ...
    req = urllib.request.Request(
        f"{panel_url}/functions/v1/contaazul-push-tokens",
        data=payload,
...
def main():
    ...
    if not client_id or not client_secret or not refresh_token:
        _out({"ok": False, "skill": SKILL_NAME,
              "error": "CONTAAZUL_CLIENT_ID, CONTAAZUL_CLIENT_SECRET ou CONTAAZUL_REFRESH_TOKEN ausentes"})
        sys.exit(1)

    expiry = float(os.environ.get("CONTAAZUL_TOKEN_EXPIRY", "0"))
