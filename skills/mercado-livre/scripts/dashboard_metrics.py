#!/usr/bin/env python3
"""Dashboard metrics para skill mercado-livre — Agente CFO. Contrato plano canônico (FIX-KPI).
...
def get_metrics() -> dict:
    ...
    try:
        ...
        try:
            ...
            items = orders.get("results", []) if isinstance(orders, dict) else (orders or [])
            out["ecommerce_revenue_month_brl"] = round(sum(float(o.get("total_amount", 0) or 0) for o in items), 2)
        except Exception:
            degraded = True
