#!/usr/bin/env python3
"""Dashboard metrics para skill mercado-livre — Agente CFO. Sprint INT-2."""
import sys, os, json
...
def get_metrics() -> dict:
    ...
    has_creds = _load_env()
    if not has_creds or not os.environ.get("ML_ACCESS_TOKEN"):
        base["health"] = "credential_invalid"
        base["error"] = f"Secrets não encontrados ou ML_ACCESS_TOKEN ausente: {SECRETS_FILE}"
        return base
    ...
    try:
        ...
        try:
            ...
            items = orders.get("results", [])
            revenue = sum(float(o.get("total_amount", 0)) for o in items)
