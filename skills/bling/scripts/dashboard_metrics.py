#!/usr/bin/env python3
"""Dashboard metrics para skill bling — Agente CFO. Sprint INT-2."""
import sys, os, json
...
def get_metrics() -> dict:
    ...
    has_creds = _load_env()
    if not has_creds or not os.environ.get("BLING_ACCESS_TOKEN"):
        base["health"] = "credential_invalid"
        base["error"] = f"Secrets não encontrados ou BLING_ACCESS_TOKEN ausente: {SECRETS_FILE}"
        return base
    ...
    try:
        ...
        # Saldo (balance)
        try:
            saldo_resp = client.get_saldo()
            metrics["balance_brl"] = round(float(saldo_resp.get("saldo", 0.0)), 2)
        except Exception:
