#!/usr/bin/env bash
set -euo pipefail

SKILL_NAME="holdprint"
SECRETS_FILE="${HOME}/.openclaw/secrets/${SKILL_NAME}.env"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

[[ -f "$SECRETS_FILE" ]] && source "$SECRETS_FILE" 2>/dev/null || true

echo "=== doctor.sh [${SKILL_NAME}] ==="

if [[ -z "${HOLDPRINT_API_KEY:-}" ]]; then
    echo "❌ ${SKILL_NAME}: HOLDPRINT_API_KEY ausente"
    echo "   Execute: bash $(dirname "$0")/connect.sh"
    exit 1
fi
echo "✅ API Key presente"

if python3 "$SCRIPT_DIR/holdprint_client.py" company_info >/dev/null 2>&1; then
    echo "✅ ${SKILL_NAME}: API acessível (https://api.holdworks.ai)"
    exit 0
else
    echo "❌ ${SKILL_NAME}: falha na conexão (API Key inválida ou API fora do ar)"
    exit 1
fi
